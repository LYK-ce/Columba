...
5. **长时间任务**：训练等长时间运行的任务需要使用后台运行方式，否则会因超时被中断或Agent关闭导致任务终止

### 长时间任务处理

由于Agent存在空闲超时机制（默认5分钟无活动后关闭），长时间运行的任务需要脱离Shell独立运行：

**Windows**：
```bash
start /b python train.py > train.log 2>&1
```
...
这样即使Agent关闭，任务也会在后台继续执行。用户可以稍后发送邮件查询日志文件内容获取执行结果。
